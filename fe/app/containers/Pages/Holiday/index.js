...
const Holiday = () => {
  ...
  const handlePutHoliday = async () => {
    ...
    await putHoliday(baseApiUrl, id, body);
  ...
  const handlePostHoliday = async () => {
    ...
    await postHoliday(baseApiUrl, body);
    setOpenNotification(true);
    setReloadKey((prevCount) => prevCount + 1);
    handleTabValueProps(0);
  };
  ...
  const handleDeleteProcessing = async (value) => {
    const id = value[0]
    await deleteHoliday(baseApiUrl, id)
  }

  const baseApiUrl = useSelector(state => state.env.BASE_API_URL);

  useEffect(async () => { setData(await fetchVerify(baseApiUrl)) }, [formData, tabValue, detailFormData]);
